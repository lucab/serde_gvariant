...
fn test_maybe_string_example() {
    ...
    let ser: Vec<u8> = serde_gvariant::to_vec(&decoded).expect("String ser");
    let de: Option<String> = serde_gvariant::from_slice(&encoded[..]).expect("String de");
    let sig = serde_gvariant::describe(&decoded, None).expect("String describe");
    assert_eq!(ser, encoded);
    assert_eq!(de, decoded);
    assert_eq!(sig, sig_type);
}
